...
class EgyptCheckoutScreen extends StatelessWidget {
  const EgyptCheckoutScreen({
    super.key,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: MultiBlocProvider(
        providers: [
          // BlocProvider(
          //   create: (context) => EgyptLayoutBloc()..getDataFromDataBase(),
          // ),
          BlocProvider(
            create: (context) => EgyptCartBloc(),
          ),
          BlocProvider(
            create: (context) => EgyptCheckoutBloc()
              ..productsInCart(EgyptLayoutBloc.get(context).cart),
          ),
        ],
        child: BlocConsumer<EgyptCheckoutBloc, EgyptCheckoutStates>(
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                      ...
                      builder: (context, state) {
                        return ListView.separated(
                          itemBuilder: (context, index) => cartItem(
                            context: context,
                            model: EgyptLayoutBloc.get(context).cart[index],
                          ),
                          separatorBuilder: (context, index) => SizedBox(
                            height: MediaQuery.of(context).size.height /
                                AppSize.s60,
                          ),
                          itemCount: EgyptLayoutBloc.get(context).cart.length,
                        );
                      },
                    ),
                  ),
                  ...
                  BlocBuilder<EgyptCartBloc, EgyptCartStates>(
                    builder: (context, state) {
                      return Row(
                  ...
                  state is CheckoutLoadingState
                      ...
                      : SharedWidget.defaultButton(
                          ...
                          onPressed: () {
                            EgyptCheckoutBloc.get(context).checkout(
                              context: context,
                              fullName: fullName,
                              email: email,
                              phone: phone,
